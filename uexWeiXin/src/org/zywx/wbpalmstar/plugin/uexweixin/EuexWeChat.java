		...
		weChatCallBack = new WeChatCallBack() {
			...
			public void callBackShareResult(int message) {
				if (code == 1) {//类型代表是文字类型
                    ...
                    jsCallbackAsyn(CB_SHARE_TEXT_CONTENT, 0, EUExCallback.F_C_INT,
                            message==0?"0":"1");
  				} else if (code == SHARE_IMAGE_CONTENT_CODE) {
                    jsCallbackAsyn(CB_SHARE_IMAGE_CONTENT, 0, EUExCallback.F_C_INT,
                            message==0?"0":"1");
  				} else if (code == SHARE_LINK_CONTENT_CODE) {
                    jsCallbackAsyn(CB_SHARE_LINK_CONTENT, 0, EUExCallback.F_C_INT,
                            message==0?"0":"1");
  				}
